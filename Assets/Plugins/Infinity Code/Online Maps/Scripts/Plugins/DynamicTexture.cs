...
namespace OnlineMaps
{
    ...
    public class DynamicTexture: MonoBehaviour
    {
        ...
        public void Start()
        {
            AdjustToScreen adjustToScreen = GetComponent<AdjustToScreen>();
            if (adjustToScreen && adjustToScreen.enabled) return;
            ...
            width = Mathf.RoundToInt(width / (float)Constants.TileSize) * Constants.TileSize;
            height = Mathf.RoundToInt(height / (float)Constants.TileSize) * Constants.TileSize;
