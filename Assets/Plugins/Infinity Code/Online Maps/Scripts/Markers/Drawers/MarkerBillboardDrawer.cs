...
namespace OnlineMaps
{
    ...
    public class MarkerBillboardDrawer : Marker2DMeshDrawer
    {
        ...
        /// Size of markers
        /// </summary>
        public float marker2DSize => control.marker2DSize;
        ...
        private void OnDrawMarkers()
        {
            ...
            float scaleX = size.x / map.buffer.renderState.width * marker2DSize;
            float scaleZ = size.z / map.buffer.renderState.height * marker2DSize;
            Vector3 offset = map.transform.rotation * (Vector3.up * size.x / 64);
            ...
            foreach (Marker2D marker in control.marker2DManager)
            {
                ...
                float sx = scaleX * marker.scale;
                float sz = scaleZ * marker.scale;
                float s = Mathf.Max(sx, sz);

                markerBillboard.transform.localScale = new Vector3(s, s, s);
                markerBillboard.transform.position = control.LocationToWorld3(p.x, p.y, r) + offset;
