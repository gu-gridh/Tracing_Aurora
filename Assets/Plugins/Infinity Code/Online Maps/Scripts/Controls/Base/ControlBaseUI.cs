...
namespace OnlineMaps
{
    ...
    public abstract class ControlBaseUI<T> : ControlBase2D where T: MaskableGraphic
    {
        ...
        public override Vector2 LocationToScreen(double lng, double lat)
        {
            ...
            p.x = mapRect.center.x + mapRect.width * p.x;
            p.y = mapRect.center.y + mapRect.height * p.y;
        ...
        public override bool ScreenToTile(Vector2 position, out TilePoint tilePoint)
        {
            ...
            {
#endif
            if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(image.rectTransform, position, worldCamera, out point))
            {
                return false;
            }
#if CURVEDUI
