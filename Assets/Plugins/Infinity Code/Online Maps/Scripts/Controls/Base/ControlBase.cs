...
namespace OnlineMaps
{
    ...
    public abstract class ControlBase : MonoBehaviour, ISavableAdvanced
    {
        ...
        /// <summary>
        /// Event that occurs when need to check if the cursor is on a UI element.
        /// </summary>
        public Predicate<Vector2> OnCheckCursorOnUIElement;
        ...
        public bool IsCursorOnUIElement(Vector2 position)
        {
            ...
            if (OnCheckCursorOnUIElement != null) return OnCheckCursorOnUIElement(position);
            
#if !IGUI && ((!UNITY_ANDROID && !UNITY_IOS) || UNITY_EDITOR)
            if (GUIUtility.hotControl != 0) return true;
#endif
            if (UIToolkitHelper.IsCursorOverInteractiveElement(position)) return true;
        ...
        public virtual Vector2d LocationToLocal(double lng, double lat)
        {
            ...
            Vector2d d = p1 - lastState.centerMercator;
            double scale = (1 << lastState.intZoom) * Constants.TileSize / lastState.zoomFactor;
            return d * scale;
        }
