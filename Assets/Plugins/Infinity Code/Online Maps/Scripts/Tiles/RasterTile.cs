...
namespace OnlineMaps
{
    ...
    public class RasterTile : Tile
    {
        ...
        public string trafficURL
        {
            get
            {
                if (!string.IsNullOrEmpty(_trafficURL)) return _trafficURL;

                if (trafficProvider.isCustom)
                {
                    _trafficURL = Regex.Replace(map.customTrafficProviderURL, @"{\w+}", CustomTrafficProviderReplaceToken);
                }
                else
                {
                    _trafficURL = trafficProvider.GetURL(this);
                }
        ...
        public override string url
        {
            get
            {
                ...
                if (mapType.isCustom)
                {
                    _url = Regex.Replace(map.customProviderURL, @"{\w+}", CustomProviderReplaceToken);
                }
                else
                {
                    _url = mapType.GetURL(this);
                }
        ...
        private void MergeColors()
        {
            try
            {
                ...
                {
                    Color32 c1 = _colors[i];
                    Color32 c2 = labelColors[i];
                    float a = c2.a / 255f;
                    
                    c1.r = (byte)((c2.r - c1.r) * a + c1.r);
                    c1.g = (byte)((c2.g - c1.g) * a + c1.g);
                    c1.b = (byte)((c2.b - c1.b) * a + c1.b);
                    c1.a = 255;
                    mColors[i] = c1;
                }
        ...
        public bool OnLabelDownloadComplete()
        {
            ...
            if (status != TileStatus.loaded) return false;
