...
namespace OnlineMapsDemos
{
    public class Navigation : MonoBehaviour
    {
        ...
        /// <summary>
        /// Should the marker snap to the route (stick to the road).
        /// </summary>
        ...
        {
            pointChanged = false;
            GoogleRoutingResult.RouteLegStep step = steps[currentStepIndex];
            ...
            if (p1 != p2)
            {
                ...
                if (p != p2)
                {
                    ...
                    if (dist < updateRouteAfterKm)
                    {
                        ...
                        lastPointOnRoute = snapToRoad ? p : location;
                        return true;
                    }
                }
            }
            ...
            for (int i = currentStepIndex; i < steps.Length; i++)
            {
                ...
                for (int j = pointIndex; j < polyline.Length - 1; j++)
                {
                    ...
                    dist = p.Distance(location);
                    if (dist > updateRouteAfterKm) continue;
                    ...
                    timeToUpdateRoute = float.MinValue;
                    lastPointOnRoute = snapToRoad ? p : location;
                    if (!useUserLocation) LookToLocation(p, p2);
                    return true;
                }
